...
create/list/read, saved research project create/list/read/run/run-due, and label
create. Opportunities are generated by processing pipelines and exposed through
read/export/regenerate/enhance endpoints; scans and labels do not have full CRUD
in this release. Scheduling is explicit through `run-due` so operators can use
cron, GitHub Actions, a worker, or the local CLI without hiding background jobs
